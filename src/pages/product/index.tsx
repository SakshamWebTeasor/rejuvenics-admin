...
export const getServerSideProps: GetServerSideProps = async ({ req, res, query }) => {
  ...
  //   console.log("from product index resp", session);
  if (!session) {
    return {
      redirect: {
        destination: '/',
  ...
  try {
    ...
    console.log("from product index resp", response);
...
export default function Product({ isAll, data, total_pages, current_page, token }: ProudctsProps) {
    ...
    <>
      <div className="main-page">
        ...
        <div className="table-data mt-3">
          <Table>
            <tbody>
              ...
              {state.data.length > 0 ? state.data.map((d: ProductModel, i: number) => <tr key={i}>
                <td>
        ...
        <div className="pagination-section">
          ...
          <Pagination>
            {state.current_page > 1 && <Pagination.Prev as={Link} href={`/product/${state.current_page - 1}?type=all`} />}
            {new Array(state.total_pages).fill(0).map((_, i) => <Pagination.Item as={Link} key={i} active={i + 1 === state.current_page ? true : false} href={`/product/${i + 1}?type=all`}>{i + 1}</Pagination.Item>)}
            {state.current_page !== state.total_pages && <Pagination.Next as={Link} href={`/product/${state.current_page + 1}?type=all`} />}
          </Pagination> : <></>}
          {!checked && state.data.length > 0 ? <Pagination>
            {state.current_page > 1 && <Pagination.Prev as={Link} href={`/product/${state.current_page - 1}`} />}
            {new Array(state.total_pages).fill(0).map((_, i) => <Pagination.Item as={Link} key={i} active={i + 1 === state.current_page ? true : false} href={`/product/${i + 1}`}>{i + 1}</Pagination.Item>)}
            {state.current_page !== state.total_pages && <Pagination.Next as={Link} href={`/product/${state.current_page + 1}`} />}
          </Pagination>
           : <></>}
        </div>
      </div>
      <AddProduct show={isAddOpen} handleClose={handleAddForm} token={token} />
      <DelCommon
        show={isDelOpen}
